...
for epoch in range(max_epochs):
    batch_data = {
        'tensors': [], 'log_probs': [], 'values': [], 'rewards': [], 'actions': [], 'entropies': [], 'agent_positions': [],
        'masks': [], 'dones': []
    }
    batch_returns = []
    episode_lengths = []
    ...
    while timesteps < batch_timesteps:
        ...
        tensors, lp, vals, rews, acts, ents, agent_positions, masks, dones, path = data
        ...
        batch_data['agent_positions'].extend(agent_positions)
    ...
    actor_loss, critic_loss, entropy_loss = solver.update(
        ...
        batch_data['agent_positions'], batch_data['masks'], batch_data['dones']
    )
